...
def chat_with_claude():
    ...
    try:
        while True:  # Main chat loop
            ...
            # Tool handling loop
            while True:  # This loop handles multiple tool calls
                    ...
                    try:
                        ...
                        if e.status_code == 429:  # Rate limit
                            ...
                            try:
                                ...
                                retry_after = retry_delay  # Fallback to exponential
                                
                            print(f"Rate limited. Retry after: {retry_after}s | Reset at: {reset_time}")
                            time.sleep(retry_after)
                            continue  # Retry immediately after waiting
                            
                        else:
                            print(f"API error {e.status_code}: {e.message}")
                            raise  # Re-raise for non-rate limit errors
                ...
                if not tool_uses:
                    # Print final response and exit tool loop
                    text_response = response.content[0].text
                    print(f"\nClaude: {text_response}")
                    break  # Exit tool loop, back to user input

                # Execute tools
                tool_results = []
                for tool_use in tool_uses:
                    result, error = tool_dispatcher(tool_use.name, tool_use.input)
