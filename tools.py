...
TOOLS_SCHEME = [
        ...
        {   # Primary tool - pushed first
            ...
            {   # Secondary tool - requires prior search
...
def find_code_patterns(pattern, file_filter):
    try:
        cmd = ["git", "grep", "-n", "-I", "--color=never", "-e", pattern, "--", file_filter]
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=5)
        lines = result.stdout.splitlines()[:5]  # Limit to 5 results
        if not lines:
            return "No matches found", None
        # Each line is already in the format: file_path:line_number:line_content
        return "\n".join(lines), None
    except Exception as e:
        return f"Search failed: {str(e)}", None

def read_code_snippet(file_path, line_numbers):
