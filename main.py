...
def claude_suggest_fix(issue, repo_url, repo_path, username, token, tools_scheme, model):
    ...
    # Reset main to clean state before new branch
    ...
    if branch_name in repo.heads:
        ...
        repo.git.reset('--hard', 'origin/main')  # Reset existing branch
    else:
...
def main():
    models = {"Haiku": "claude-3-5-haiku-20241022", "Sonnet": "claude-3-7-sonnet-20250219"}
    ...
    for issue in issues:
        ...
        result = claude_suggest_fix(issue, repo_url, repo_path, username, token, tools_scheme, models["Haiku"])
        
        if result["success"]:
            print(f"Success! Create PR at: {result['pr_url']}")
        else:
